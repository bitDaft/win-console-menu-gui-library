...
void consoleMenu::setcolor(unsigned short m = cGRAY,
                           ...
                           short          y = 0)
{
    FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE), m, t, (COORD)
    {
        x, y
    }, (PDWORD)&t);
    gotoxyz(0,0);
...
consoleMenu::consoleMenu(char str[])
{
    ...
    //setOptions(0, 0,' ', 0, ENABLE_PLAIN, ALIGN_CENTER | SELECT_HIGHLIGHT, USE_MOUSE);
...
int consoleMenu::Mset()
{
    ...
    {
        if(menuItemVisual&ALIGN_CENTER)
            menuItemVisual&=~ALIGN_LEFT;
        ...
        if(menuItemVisual&SELECT_HIGHLIGHT)
        {
            ...
            menuItemVisual&=~SELECT_BOX;
        else menuItemVisual|=SELECT_BOX;
    }
    ...
    {
        if(ci>10)
            return LINE_MENU_OVERFLOW;
    }
    else
    {
        if((menuWidth+x)>80||(menuHeight+y)>25)
            return MENU_OVERFLOW;
    }
...
int consoleMenu::newItem(char *p, void (*t)(),consoleMenu *mm)
{
    ...
    else
    {
        ...
        if(menuItemVisual&ALIGN_LINE)
            ptr->next->ix = ptr->ix+ptr->length+1;
        else
            ptr->next->ix = x;
        ptr=ptr->next;
    }
...
int consoleMenu::paintMenu()
{
    ...
    if(menuItemVisual&ALIGN_LINE)
    {
        ...
        FillConsoleOutputCharacter(GetStdHandle(STD_OUTPUT_HANDLE),' ',80,(COORD)
        {
            tx,ty
        },&t);
    ...
    if(mode&ALIGN_LINE)
    {
        ...
        if(ir.dwMousePosition.Y==0)
        {
            ...
            *o=0;
            ...
            {
                if (mode & SELECT_HIGHLIGHT) flags = BACKGROUND_BLUE | FOREGROUND_BLUE |
                                                         FOREGROUND_GREEN |
                                                         FOREGROUND_INTENSITY |
                                                         FOREGROUND_RED;
                else if (mode & SELECT_TEXT) flags = FOREGROUND_BLUE | FOREGROUND_GREEN |
                                                         FOREGROUND_INTENSITY |
                                                         FOREGROUND_RED|mnBG;
                FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                                           flags,
...
//            if(py==0)
             ...
             (ir.dwMousePosition.X > x) && (ir.dwMousePosition.X < x + w-1))
    {
        ...
        if (mode & SELECT_BOX) *o = (ir.dwMousePosition.Y - iy) %
                                        2 ? -1:(ir.dwMousePosition.Y -iy+1)/ 2 ;
...
//                {
//                    if(ir[i].Event.MouseEvent.dwMousePosition.Y!=0)return NOT_ON_MENU;
//                }
//                else
//                {
//                    return NOT_ON_MENU;
...
//            }
            ...
            case MOUSE_EVENT:
                if (MouseOrKey & USE_MOUSE)
                {
                    ...
                    int tt=handleMouse(ir[i].Event.MouseEvent, start,
                                       x, y,
                    ...
                    if (tt == MOUSE_LEFT_PRESS)
                    {
                        ...
                        if (opt == -1)
                        {
                            if(childFlag&IS_CHILD&&(isChildOf->menuItemVisual)&ALIGN_LINE)
                            {
                                ...
                                //instead of clearing screen ,have the stored output buffer restored

                                //clearScreen(0,1);       // change these 3  to store the input buffer
                                isChildOf->paintBackground();      // and then restore it ,it will allow to work
                                isChildOf->paintMenu(); // with pictures as backgrounds and real time
                                                        // menu's
...
//                        {
//                            //TODO
//                            //save the current output buffer to have it restored if required
//                        }
//                        else
//                        {
//                            //clearScreen(0,0);
...
//                        }
                        ...
                        else
                        {
                            ...
                            {
                                if((isChildOf->menuItemVisual)&ALIGN_LINE)
                                    clearScreen(0,1);//more like save the buffer ,do the function,restore the buffer
                                else
                                {
                                    if(ptr->next==NULL)
                                    {
...
{
    FillConsoleOutputCharacter(GetStdHandle(STD_OUTPUT_HANDLE), ' ', t,(COORD)
    {
        x,y
    }, &t);
    FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE),
                               FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED, t, (COORD)
    {
        x,y
    },&t);
    gotoxyz(x,y);
