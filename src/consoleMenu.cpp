    ...
    (ir.dwMousePosition.X >= x) && (ir.dwMousePosition.X < x + w ))
      {
        ...
        *o = ir.dwMousePosition.Y - iy;
        ...
        {
          FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE), mnBG, w, (COORD) {x, py }, &t);
        ...
        FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE), flags, w , (COORD) {x, py }, &t);
...
int invMenu::paintView()
{
  ...
  for (size_t j = 0; j < nrec; j++) {
    ...
    ty=rcd->iy;
    if (snCount) {
      FillConsoleOutputCharacter(GetStdHandle(STD_OUTPUT_HANDLE),sl[0],1,(COORD){x,ty},&t);
      // if(sl[1]>'0')
...
int invMenu::selectView()
{
  ...
  for (register unsigned int i = 0; i < numOfEntries; i++)
  {
    ...
    {
    case MOUSE_EVENT:;
    	int tt = handleMouse(ir[i].Event.MouseEvent,NULL,x,y,height,width,&opt,0,viewColor,top);
